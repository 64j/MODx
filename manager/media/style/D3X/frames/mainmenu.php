...
if($modx->hasPermission('view_eventlog') || $modx->hasPermission('view_eventlog') || $modx->hasPermission('logs')) {
...
if($modx->hasPermission('exec_module')) {
	if ($_SESSION['mgrRole'] != 1) {
		$rs = $modx->db->query('SELECT DISTINCT sm.id, sm.name, mg.member
				...
				WHERE (mg.member IS NULL OR mg.member = '.$modx->getLoginUserID().') AND (sma.usergroup IS NULL OR sma.usergroup = '.$_SESSION['mgrRole'].') AND sm.disabled != 1');
	} else {
